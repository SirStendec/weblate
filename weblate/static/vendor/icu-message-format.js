...
},{"./plurals":3,"format-message-formats":1,"lookup-closest-locale":5}],3:[function(require,module,exports){
// @flow
'use strict'
...
const TAG_TYPE = '<,TAG,>',
  NUMERIC_TYPES = [
    'number', 'plural', 'selectordinal', 'spellout'
  ],
  DATE_TYPES = [
    'date', 'time', 'datetime'
  ],
...
function extract(ast, variables = {}) {
  ...
  for(const token of ast) {
    ...
    else if ( (data.type === 'date' && type === 'time') ||
        (data.type === 'time' && type === 'date') )
      data.type = 'datetime';

    data.is_number = update_maybe_value(NUMERIC_TYPES.includes(type), data.is_number);
    ...
    if ( type === 'select' && token[2] ) {
      ...
      if ( data.choices )
        data.choices = data.choices.concat(choices);
      else
    ...
    if ( data.type && INPUT_TYPES[data.type] )
      ...
      data.editor_type = 'number';
    else if ( data.is_date )
      data.editor_type = data.type;
    else if ( data.is_tag && data.is_empty )
      data.editor_type = false;

    // Descend into sub-asts.
    for(let i=2; i < 5; i++) {
      if ( typeof token[i] === 'object' && ! Array.isArray(token[i]) )
        for(const val of Object.values(token[i]))
          extract(val, variables)
    }
  }
...
function interpret(ast, locale, variables) {
  return fmInterpret.toParts(ast, locale, {
    [TAG_TYPE]: (input, locale) => {
      const children = input && input[2] && input[2].children;
      return (fn, args) => {
        if ( typeof fn === 'function' )
          return fn(children && children(args));
        return null;
      }
    }
  })(variables);
